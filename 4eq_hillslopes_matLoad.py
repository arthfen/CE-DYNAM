...
import math, os, glob, sys, numpy as np, scipy.sparse.linalg, scipy.optimize, datetime, pandas as pd, cPickle as pickle, subprocess
# import cupy as cp
...
#					x_cut = spm(A_gpu, B_cut, x_cut, mdays[mk])
					...
					if mk == 11:
						...
						if(yk < 1869): year_new = yk + 1
						else: year_new = 1860

						x = ratio_calc(x, yr_old, year_new)
...
#			x_cut = spm(A_gpu, B_cut, x_cut, mdays[mo])
		...
		for mk in months:
			os.system('cd /home/users/afendric/CE_DYNAM_WithoutCCs_EF1/matSave/; ./submit-auto_' + set_eros + set_depo + '.sh ' + str(yr) + ' ' + str(mk) + '; cd -')
